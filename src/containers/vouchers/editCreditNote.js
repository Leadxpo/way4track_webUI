...
const EditCreditNoteForm = (props) => {
  ...
  useEffect(() => {
    if (isEditMode && item) {
      const formattedDate = item.generationDate
        ? new Date(item.generationDate).toISOString().split('T')[0]
        : '';
      ...
      setFormData({
        ...
        amount: item.amount || '',
        voucherType: item.voucherType || 'CREDITNOTE',
