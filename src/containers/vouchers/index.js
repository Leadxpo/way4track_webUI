...
const Vouchers = () => {
  ...
  const handleSearch = () => {
    const filtered = voucherList.filter((item) => {
      const matchID = voucherID ? String(item.voucherId).includes(voucherID) : true;
      const matchType = selectedVoucherType ? item.voucherType === selectedVoucherType : true;
  ...
  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        ...
        <button onClick={handleNavigation} className="bg-yellow-300 text-black font-semibold px-4 py-2 rounded-full shadow-md hover:brightness-95">
          + Create Vouchers
        </button>
      </div>
