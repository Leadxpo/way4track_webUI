...
const Home = () => {
    ...
    const [tableColumns, setTableColumns] = useState(Object.keys(totalProducts[0]));
    ...
    const branchesData = [
        {
          branch: 'Branch 1',
          background: 'linear-gradient(180deg, #CE0000 0%, #D50000 50%, #C00000 75%, #B50000 87.5%, #A70000 93.75%, #8F0404 100%)',
          data: [
            { month: 'Jan', profit: 20 },
            { month: 'Feb', profit: 30 },
            { month: 'Mar', profit: 25 },
        ...
        {
          branch: 'Branch 2',
          background: 'linear-gradient(180deg, #12A651 0%, #0C7338 50%, #0A5A2C 75%, #084D25 87.5%, #084622 93.75%, #074321 96.87%, #074220 98.44%, #07401F 100%)',
          data: [
            { month: 'Jan', profit: 40 },
            { month: 'Feb', profit: 35 },
            { month: 'Mar', profit: 50 },
        ...
        {
          branch: 'Branch 4',
          background: 'linear-gradient(180deg, #0033CB 0%, #002698 50%, #00207F 75%, #001C72 87.5%, #001B6C 93.75%, #001965 100%)',
          data: [
            { month: 'Jan', profit: 50 },
            { month: 'Feb', profit: 55 },
            { month: 'Mar', profit: 60 },
    ...
    const columns = ['Ticket Number', 'Purpose', 'Branch', 'Date', 'Amount', 'Payment Status'];
    ...
    const tableMockData = [
    { Name: 'John Doe', Age: 28, Position: 'Engineer', Department: 'Development' },
    { Name: 'Jane Smith', Age: 34, Position: 'Manager', Department: 'Operations' },
    { Name: 'Michael Lee', Age: 25, Position: 'Analyst', Department: 'Finance' },
    { Name: 'Sophia Johnson', Age: 30, Position: 'Designer', Department: 'Marketing' },
    { Name: 'David Brown', Age: 29, Position: 'Developer', Department: 'Development' },
    ];
  ...
  const handleStatusChange = (e) => {
    ...
    const filtered = tableData.filter((item) => selectedBranch === '' || item.Branch === selectedBranch);
  ...
  return (
    <div className='flex flex-col space-y-16'>
        ...
        <div className="flex mt-8">
            <div className="w-3/5">
                {branch_details.map((branch)=>(
                    ...
                    <img src="logo-name-square.png" alt="Branch Logo" className="w-10 h-10 object-cover" />
                    <div>
                        <p className="text-gray-800 font-semibold">Branch</p>
                        <p className="text-green-700 font-semibold">{branch.branch_name}</p>
                    </div>
                    </div>
                    <div className="text-left">
                    <p className="text-gray-500">Service Sales</p>
                    <p className="text-gray-800 font-bold">{branch.service_sales}</p>
                    </div>
                    <div className="text-left">
                    <p className="text-gray-500">Product Sales</p>
                    <p className="text-gray-800 font-bold">{branch.product_sales}</p>
                    </div>
                    <div className="text-left">
                    <p className="text-gray-500">Total Sales</p>
                    <p className="text-gray-800 font-bold">{branch.total_sales}</p>
                    </div>
                </div>
        ...
        <div className="flex justify-between space-x-4 mt-12">
            {CardData.map((card) => (
                <div onClick={()=>handleCardClick(card.title)}><TotalCountCard data={card}/></div>
            ))}
        </div>
