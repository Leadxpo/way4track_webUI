...
const BranchManagerHome = () => {
  ...
  const fetchAssertsCardData = async () => {
    try {
      ...
      if (payload.branch) {
        response = await ApiService.post(
          '/dashboards/getWareHouseProductDetailsByBranch',
          payload
        );
      }
      if (response.status) {
  ...
  const fetchStaffData = async () => {
    try {
      ...
      if (response?.status) {
        setStaffData(
          Array.isArray(response.data.result[0]) ? response.data.result[0] : []
        );
      } else {
  ...
  const ReceivedPayments = async () => {
    try {
      ...
      if (payload.branchName) {
        response = await ApiService.post(
          '/technician/getSucessPaymentsForTable',
          payload
        );
      }
      // Ensure response.data is always an array
  ...
  const PendingAmount = async () => {
    try {
      ...
      if (payload.branchName) {
        response = await ApiService.post(
          '/technician/getPendingPaymentsForTable',
          payload
        );
      }
      // Ensure response.data is always an array
  ...
  const TotalStaffDetails = async () => {
    try {
      ...
      if (response?.status) {
        ...
        setTotalStaffDetails(
          Array.isArray(response.data.staff) ? response.data.staff : []
        );
    ...
    <div className="p-6">
          ...
          <tbody>
            {filteredProducts.length > 0 ? (
              filteredProducts.map((row, index) => (
                <tr
                  key={index}
                  className={`${index % 2 === 0 ? 'bg-gray-200' : 'bg-white'} border-b`}
                >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6 mt-10">
        {stats.map((stat, index) => (
          <div
            key={index}
            className={`rounded-lg shadow-md p-5 border ${stat.borderColor}`}
          >
            <div
              className={`text-white p-3 rounded-t-lg font-semibold text-xl ${stat.color}`}
            >
              {stat.title}{' '}
              <span className="float-right">Total : {stat.total}</span>
            </div>
            <div className="p-4">
