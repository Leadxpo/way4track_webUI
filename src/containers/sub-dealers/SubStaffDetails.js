...
const SubStaffDetails = () => {
      ...
      try {
        ...
        if (response.status) {
          ...
          const filteredData = data
            .filter(item => item.subDealerId && item.subDealerId.id === SubStaffId)
 ...
 const handleDownload = () => {
  ...
  const handleDelete = async (subStaffId) => {
    if (!window.confirm("Are you sure you want to delete this Subdealer Staff?")) {
      return;
    }

    const payload={id:subStaffId,companyCode:initialAuthState.companyCode,unitCode:initialAuthState.unitCode}
...
branch||"-"
 ...
 {/* Stats Section */}
        ...
        <div>
          <h2 className="text-2xl font-bold mb-4 text-red-400">Sub Dealer Staff Details</h2>
...
email
                  ...
                  <td className="p-2 border text-center">{payment.phoneNumber}</td>
                                        ...
                                        {dropdownOpen === payment.id && (
                                          <div className="absolute right-0 mt-2 bg-white shadow-lg border rounded-md min-w-[150px] z-50">
                                            <ul className="text-left">
                                              <li className="p-2 hover:bg-gray-100 cursor-pointer" onClick={() => navigate("/edit-sub-staff", { state: { subdealerstaff: payment } })}>Edit</li>
                                              <li className="p-2 hover:bg-gray-100 cursor-pointer"  onClick={() => handleDelete(payment.id)}>Delete</li>
                                              <li className="p-2 hover:bg-gray-100 cursor-pointer" onClick={() => navigate("/view-sub-staff", { state: { subdealerstaff: payment } })}>More Details</li>
                                            </ul>
                                          </div>
                                        )}
                                      </td>
