...
const StaffAutocomplete = ({ staffData, formData, setFormData }) => {
  ...
  useEffect(() => {
    const selected = staffData.find((staff) => staff.id === formData.requestTo);
    setInputValue(selected ? selected.name : '');
  }, [formData.requestTo, staffData]);
  ...
  const handleOptionClick = (staff) => {
    ...
    setFormData((prev) => ({
      ...prev,
      requestTo: staff.id, // Save the ID only
    }));
    setShowDropdown(false);
  };
